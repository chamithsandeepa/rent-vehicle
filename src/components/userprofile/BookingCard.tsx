...
import JourneyCompletedModal from "@/components/userprofile/JourneyCompletedModal";
import CancelBookingModal from "@/components/userprofile/CancelBookingModal"; // ✅ Import cancel modal
...
const BookingCard = ({
  ...
  const [openCancelModal, setOpenCancelModal] = useState(false); // ✅ New cancel modal state
  ...
  const handleEndJourney = () => {
    onEndJourney?.(booking.id);
    setOpenJourneyModal(true);
  };

  const handleCancelClick = () => {
    setOpenCancelModal(true); // ✅ Open cancel modal
  };

  const handleConfirmCancel = () => {
    onCancel?.(booking.id);
    setOpenCancelModal(false); // Close after action
  };
  ...
  return (
    <>
        ...
        <CardContent className="p-5">
          ...
          {isFuture && (
            <div className="flex flex-col gap-2 mt-4">
              ...
              <Button
                onClick={handleCancelClick} // ✅ open cancel modal
                variant="ghost"
      ...
      <JourneyCompletedModal
        ...
        onClose={() => setOpenJourneyModal(false)}
      />
